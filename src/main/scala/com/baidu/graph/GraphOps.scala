...
object GraphOps {
  ...
  //
  lazy val createTicEdgeWithFeat = (srcId: Long, srcFeat: String, dstId: Long, dstFeat: String, weight: Double) => {
    Array(
      (srcId, Array((dstId, (dstFeat, weight)))),
      (dstId, Array((srcId, (srcFeat, weight))))
    )
  }
